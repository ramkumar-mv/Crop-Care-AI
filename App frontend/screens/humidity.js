...
const Humidity = ({ navigation }) => {
    const { foodItems, setFoodItems, userID, humAmount } =
    ...
    const addItem = async () => {
        if (value != null && serving > 0) {
            ...
            if (!found) {
                let result = humAmount[item.type][1] * item.servings;
                ...
                const dayDocRef = doc(
                    db,
                    `userInfo/${userID}/humidityTotals`,
                    dateString
                );
    ...
    const handleDelete = async (item) => {
        ...
        let result = humAmount[item.type][1] * item.servings * -1;
        setTotal(total + result);
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/humidityTotals`, dateString);
        await updateDoc(dayDocRef, docData);
    };
    ...
    const handleConfirm = async (date) => {
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/humidityTotals`, conversion);
    ...
    useEffect(async () => {
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/humidityTotals`, todayDate);
        const dayDocSnap = await getDoc(dayDocRef);
        ...
        <>
            ...
            >
                ...
                <View style={{ marginTop: 20 }}>
                    ...
                    >
                        <Text style={{ fontSize: 20 }}>{total} g.m-3</Text>
                    </ProgressCircle>
                </View>
                <Text
                    style={{
                ...
                >
                    {Object.keys(humAmount).map((item) => (
                        <TouchableOpacity>
                            <View
                                style={{
                                    marginTop: 20,
