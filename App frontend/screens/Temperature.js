...
const WeatherScreen = ({ navigation }) => {
    const { foodItems, setFoodItems, userID, tempAmount } =
    ...
    const addItem = async () => {
        if (value != null && serving > 0) {
            ...
            if (!found) {
                let result = tempAmount[item.type][1] * item.servings;
                ...
                const dayDocRef = doc(
                    db,
                    `userInfo/${userID}/temperatureTotals`,
                    dateString
                );
    ...
    const handleDelete = async (item) => {
        ...
        let result = tempAmount[item.type][1] * item.servings * -1;
        setTotal(total + result);
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/temperatureTotals`, dateString);
        await updateDoc(dayDocRef, docData);
    };
    ...
    const handleConfirm = async (date) => {
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/temperatureTotals`, conversion);
    ...
    useEffect(async () => {
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/temperatureTotals`, todayDate);
        const dayDocSnap = await getDoc(dayDocRef);
        ...
        <>
            ...
            >
                ...
                <View style={{ marginTop: 20 }}>
                    ...
                    >
                        <Text style={{ fontSize: 20 }}>{total} °C</Text>
                    </ProgressCircle>
                </View>
                <Text
                    style={{
                ...
                >
                    {Object.keys(tempAmount).map((item) => (
                        <TouchableOpacity>
                            <View
                                style={{
                                    marginTop: 20,
    ...
    >
        Temperature Value on this date
    </Text>
