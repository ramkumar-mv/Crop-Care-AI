...
const PhosScreen = ({ navigation }) => {
    const { foodItems, setFoodItems, userID, phosAmount } =
    useContext(StateContext);
const [isDatePickerVisible, setDatePickerVisibility] = useState(false);
const [currDate, setCurrDate] = useState(new Date());
const [items, setItems] = useState([
    { label: "Phosphorous", value: "Phosphorous" },
...
const addItem = async () => {
    if (value != null && serving > 0) {
        ...
        if (!found) {
            let result = phosAmount[item.type][1] * item.servings;
...
const handleDelete = async (item) => {
    ...
    let result = phosAmount[item.type][1] * item.servings * -1;
    setTotal(total + result);
    ...
    const dayDocRef = doc(db, `userInfo/${userID}/phosTotals`, dateString);
    await updateDoc(dayDocRef, docData);
};
...
const handleConfirm = async (date) => {
    ...
    const dayDocRef = doc(db, `userInfo/${userID}/phosTotals`, conversion);
...
useEffect(async () => {
    ...
    const dayDocRef = doc(db, `userInfo/${userID}/phosTotals`, todayDate);
    const dayDocSnap = await getDoc(dayDocRef);
    ...
    <>
        ...
        >
            ...
            <View style={{ marginTop: 20 }}>
                ...
                >
                    <Text style={{ fontSize: 20 }}>{total} kg/ha</Text>
                </ProgressCircle>
            </View>
            <Text
                style={{
            ...
            >
                {Object.keys(phosAmount).map((item) => (
                    <TouchableOpacity>
                        <View
                            style={{
                                marginTop: 20,
