...
const Potassium = ({ navigation }) => {
    const { foodItems, setFoodItems, userID, potasAmount } =
    ...
    const addItem = async () => {
        if (value != null && serving > 0) {
            ...
            if (!found) {
                let result = potasAmount[item.type][1] * item.servings;
                ...
                const dayDocRef = doc(
                    db,
                    `userInfo/${userID}/potassiumTotals`,
                    dateString
                );
    ...
    const handleDelete = async (item) => {
        ...
        let result = potasAmount[item.type][1] * item.servings * -1;
        setTotal(total + result);
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/potassiumTotals`, dateString);
        await updateDoc(dayDocRef, docData);
    };
    ...
    const handleConfirm = async (date) => {
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/potassiumTotals`, conversion);
    ...
    useEffect(async () => {
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/potassiumTotals`, todayDate);
        const dayDocSnap = await getDoc(dayDocRef);
    ...
    return (
        <>
            ...
            >
                ...
                >
                    {Object.keys(potasAmount).map((item) => (
                        <TouchableOpacity>
                            <View
                                style={{
                                    marginTop: 20,
