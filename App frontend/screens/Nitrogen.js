...
const Nitrogen = ({ navigation }) => {
    const { foodItems, setFoodItems, userID, nitroAmount } =
    ...
    const addItem = async () => {
        if (value != null && serving > 0) {
            ...
            if (!found) {
                let result = nitroAmount[item.type][1] * item.servings;
                ...
                const dayDocRef = doc(
                    db,
                    `userInfo/${userID}/nitrogenTotals`,
                    dateString
                );
    ...
    const handleDelete = async (item) => {
        ...
        let result = nitroAmount[item.type][1] * item.servings * -1;
        setTotal(total + result);
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/nitrogenTotals`, dateString);
        await updateDoc(dayDocRef, docData);
    };
    ...
    const handleConfirm = async (date) => {
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/nitrogenTotals`, conversion);
    ...
    useEffect(async () => {
        ...
        const dayDocRef = doc(db, `userInfo/${userID}/nitrogenTotals`, todayDate);
        const dayDocSnap = await getDoc(dayDocRef);
    ...
    return (
        ...
        <Image
            ...
            >
                ...
                >
                    {Object.keys(nitroAmount).map((item) => (
                        <TouchableOpacity>
                            <View
                                style={{
                                    marginTop: 20,
